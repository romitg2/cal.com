...
**Solution:** This issue occurs because the container tries to reach its own public hostname but cannot resolve it internally. The fix depends on your setup:

**Option 1: Use `host-gateway` to reach reverse proxy on host (recommended)**

If your reverse proxy (nginx/traefik) handles HTTPS on the host machine, use Docker's `host-gateway`:
...
services:
  calcom:
    extra_hosts:
      - "cal.yourdomain.com:host-gateway"
```

`host-gateway` (Docker 20.10+) resolves to the host machine's internal IP, allowing the container to reach your reverse proxy which then handles the HTTPS connection.

**Option 2: Configure reverse proxy headers**

Ensure your reverse proxy forwards the correct headers so NextAuth can detect the protocol:
...
> **⚠️ Do not use `127.0.0.1`** — If `NEXTAUTH_URL` uses HTTPS, the container will try to connect to `127.0.0.1:443`, but Cal.com listens on port 3000. This causes connection failures.

> **Why not use `localhost` in NEXTAUTH_URL?** Setting `NEXTAUTH_URL` to `http://localhost:3000` would fix this DNS error, but **breaks OAuth** — external providers like Google would redirect to `localhost` instead of your domain.
